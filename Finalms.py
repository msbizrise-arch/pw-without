...
def build_pw_video_url(videoUrl: str, item_id: str, video_id: str) -> str:
    """
    PW video URL mein parentId aur childId append karo.
    parentId = schedule/content item ka _id  (v2 contents response)
    childId  = videoDetails ka _id           (nested inside item)
    Ye dono DRM licence ke liye zaroori hain.
    """
    if item_id and video_id:
        sep = '&' if '?' in videoUrl else '&'
        return f"{videoUrl}{sep}parentId={item_id}&childId={video_id}"
    return videoUrl


def extract_video_url_from_item(item: dict) -> str:
    """
    v2/contents API item se video URL seedha nikalo.
    PW kai jagah URL rakhta hai — sab check karo in order.
    """
    # 1. Direct top-level url field (most common for videos)
    url = item.get('url', '')
    if url and ('cloudfront' in url or 'mpd' in url or 'm3u8' in url):
        return url

    # 2. videoDetails nested object
    vd = item.get('videoDetails') or {}
    url = vd.get('videoUrl') or vd.get('embedCode') or ''
    if url:
        return url

    # 3. Sometimes directly as videoUrl at top level
...
async def fetch_pwwp_all_schedule(session: aiohttp.ClientSession, chapter_id, selected_batch_id, subject_id, content_type, headers: Dict) -> List[Dict]:
    """
    v2/contents API se saare items fetch karo ek content_type ke liye.
    Pagination handle karta hai — jab tak data milta rahe, fetch karta rahe.
    """
    all_items = []
    page = 1
    while True:
...
async def process_pwwp_chapter_content_from_item(item: dict, content_type: str) -> dict:
    """
    v2/contents item se DIRECTLY content extract karo — no extra API call needed.

    BUG FIX: Pehle code ek extra v1/schedule-details API call karta tha har video ke liye.
    Wo API aksar empty videoDetails return karta tha isliye videos nahi aa rahe the.
    Ab hum v2/contents response ke items se seedha URL nikalte hain.
    """
    content = []
    name = item.get('topic', '') or item.get('name', '') or 'Untitled'
    item_id = item.get('_id', '')

    if content_type in ('videos', 'DppVideos'):
        videoUrl = extract_video_url_from_item(item)

        if videoUrl:
            # videoDetails se childId nikalo
            vd = item.get('videoDetails') or {}
            video_id = vd.get('_id', '') or item_id  # fallback to item_id
            final_url = build_pw_video_url(videoUrl, item_id, video_id)
            line = f"{name}:{final_url}"
            content.append(line)

    elif content_type in ('notes', 'DppNotes'):
        homework_ids = item.get('homeworkIds', []) or []
        for homework in homework_ids:
            attachment_ids = homework.get('attachmentIds', []) or []
            hw_name = homework.get('topic', '') or name
            for attachment in attachment_ids:
                att_url = attachment.get('baseUrl', '') + attachment.get('key', '')
                if att_url:
                    line = f"{hw_name}:{att_url}"
                    content.append(line)
...
async def process_pwwp_chapters(session: aiohttp.ClientSession, chapter_id, selected_batch_id, subject_id, headers: Dict):
    """
    Ek chapter ke andar ke saare content types fetch karo aur combine karo.
    videos, notes, DppNotes, DppVideos — sab.
    """
    content_types = ['videos', 'notes', 'DppNotes', 'DppVideos']

    # Saare content types ke liye ek saath fetch karo (parallel)
    all_schedule_tasks = [
    ...
    # Sab items ek list mein
    all_items = []
    for schedule in all_schedules:
        all_items.extend(schedule)

    # Ab seedha items se content nikalo — no extra API call
    content_tasks = [
        process_pwwp_chapter_content_from_item(item, item['content_type'])
    ...
    for result in content_results:
        if result:
            for ct, ct_list in result.items():
                if ct not in combined_content:
                    combined_content[ct] = []
                combined_content[ct].extend(ct_list)
...
async def get_pwwp_todays_schedule_content_details(session: aiohttp.ClientSession, selected_batch_id, subject_id, schedule_id, headers: Dict) -> List[str]:
    """
    Today's schedule ke liye v1/schedule-details API use karna padta hai
    kyunki todays-schedule API sirf schedule IDs deta hai, items nahi.
    Videos ke liye parentId/childId bhi append hoga.
    """
    ...
    if data and data.get("success") and data.get("data"):
        data_item = data["data"]
        name = data_item.get('topic') or 'Untitled'
        ...
        if video_details:
            ...
            if videoUrl:
                video_id = video_details.get('_id', '') or item_id
                final_url = build_pw_video_url(videoUrl, item_id, video_id)
                content.append(f"{name}:{final_url}\n")

        # ── Notes (homeworkIds) ──
        homework_ids = data_item.get('homeworkIds') or []
        for homework in homework_ids:
            attachment_ids = homework.get('attachmentIds') or []
            hw_name = homework.get('topic') or name
            for attachment in attachment_ids:
                att_url = attachment.get('baseUrl', '') + attachment.get('key', '')
                if att_url:
                    content.append(f"{hw_name}:{att_url}\n")

        # ── DPP Notes ──
        dpp = data_item.get('dpp')
        if dpp:
            dpp_homework_ids = dpp.get('homeworkIds') or []
...
async def pwwp_callback(bot, callback_query):
    ...
    try:
        ...
        pass  # QUERY_ID_INVALID ignore karo
