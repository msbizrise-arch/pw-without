...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8277629583:AAHFrekPPUxgY65KjbbNS5wVULsVeXrgPkg")
    ...
    async with aiohttp.ClientSession(connector=CONNECTOR) as session:
        try:
            if raw_text1.isdigit() and len(raw_text1) == 10:
                ...
                try:
                    # FIX: Use otp_headers (correct Client-Version, Integration-With: Origin)
                    async with session.post("https://api.penpencil.co/v1/users/get-otp?smsType=0", json=data, headers=otp_headers) as response:
                        resp_data = await response.json()
                        if not resp_data.get("success", True) and resp_data.get("message"):
                            await safe_edit(editable, f"**OTP Error : {resp_data.get('message')}**")
                            return
                ...
                try:
                    ...
                    async with session.post("https://api.penpencil.co/v3/oauth/token", json=payload, headers=token_headers) as response:
                        resp_json = await response.json()
                        # OTP error detection — isOTPErr logic (databasepw.py Section 4)
                        err_str = str(resp_json).lower()
                        if any(x in err_str for x in ['invalid otp', 'otp expired', 'wrong otp', 'incorrect otp']):
                            await safe_edit(editable, "**Error : Wrong or Expired OTP ❌ Please try again**")
                            return
                        if not resp_json.get("data", {}).get("access_token"):
                            await safe_edit(editable, f"**Error : {resp_json.get('message', 'Token not received')}**")
                            return
