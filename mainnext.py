...
# mainnext.py (FULL VERSION - COMPLETE BOT)
...
async def process_content(session, batch_id, subject_id, schedule_id, content_type, headers):
    ...
    if content_type in ["videos", "DppVideos"]:
        ...
        video = vd.get("videoUrl") or vd.get("embedCode")
        if video:
            result.append(f"{d.get('topic')}:{video}")
...
async def process_chapter(session, batch_id, subject_id, chapter_id, headers):
    ...
    tasks = [
        process_content(session, batch_id, subject_id, item["_id"], item["content_type"], headers)
