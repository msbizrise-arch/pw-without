...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8534420363:AAGuEuR5FLLSqwG9nOYMirLD3eso2N3pPdM")
    ...
    if data and data.get("success") and data.get("data"):
        ...
        if content_type in ("videos", "DppVideos"):
            ...
            if video_details:
                ...
                if videoUrl:
                    # Append parentId, childId, videoId for CDN playback (required by PW player)
                    parent_id = data_item.get('parentId', '')
                    child_id  = data_item.get('_id', '')
                    video_id  = video_details.get('_id', '')
                    if parent_id and child_id and video_id:
                        videoUrl = f"{videoUrl}&parentId={parent_id}&childId={child_id}&videoId={video_id}"
...
async def process_pwwp_chapters(session: aiohttp.ClientSession, chapter_id, selected_batch_id, subject_id, headers: Dict):
    ...
    all_schedules = await asyncio.gather(*all_schedule_tasks, return_exceptions=True)
...
async def process_pwwp_subject(session: aiohttp.ClientSession, subject: Dict, selected_batch_id: str, selected_batch_name: str, zipf: zipfile.ZipFile, json_data: Dict, all_subject_urls: Dict[str, List[str]], headers: Dict):
    ...
    for chapter, chapter_content in zip(chapters, chapter_results):
        ...
        if isinstance(chapter_content, Exception):
            logging.error(f"Chapter '{chapter_name}' failed: {chapter_content}")
            continue
...
async def process_pwwp(bot: Client, m: Message, user_id: int):
    ...
    async with aiohttp.ClientSession(connector=CONNECTOR) as session:
                        ...
                        with zipfile.ZipFile(f"{clean_file_name}.zip", 'w') as zipf:
                            ...
                            for i, sr in enumerate(subject_results):
                                if isinstance(sr, Exception):
                                    logging.error(f"Subject index {i} failed: {sr}")
                        
                        with open(f"{clean_file_name}.json", 'w') as f:
                            json.dump(json_data, f, indent=4)
                            
                        logging.info(f"all_subject_urls keys: {list(all_subject_urls.keys())}")
                        with open(f"{clean_file_name}.txt", 'w', encoding='utf-8') as f:
                            total_written = 0
                            for subject in subjects:
                                subject_name = subject.get("subject", "Unknown Subject").replace("/", "-")
                                if subject_name in all_subject_urls:
                                    lines = all_subject_urls[subject_name]
                                    if lines:
                                        f.write('\n'.join(lines) + '\n')
                                        total_written += len(lines)
                                else:
                                    logging.warning(f"Subject '{subject_name}' not in all_subject_urls")
                        logging.info(f"TXT file written with {total_written} total URLs")
        ...
        # session and CONNECTOR are closed automatically by async with block
...
async def process_cpwp(bot: Client, m: Message, user_id: int):
    ...
    async with aiohttp.ClientSession(connector=CONNECTOR) as session:
        try:
            ...
            await editable.edit(f"**Error : {e}**")
        # session and CONNECTOR closed automatically by async with block
